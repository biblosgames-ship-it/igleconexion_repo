          ...
          <div>
            ...
            <div className={styles.dashboardLayout}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                ...
                {/* Timeline de Crecimiento (Mi Ruta de Avance en el Grupo) */}
                <div className={styles.card}>
                  <div className={styles.cardHeader}>
                    <h2 className={styles.cardTitle}>🌱 Mi Ruta de Avance</h2>
                  </div>
                  
                  <div className={styles.timeline}>
                    {(() => {
                      const userCompletedTasks = profile?.tareas_completadas || [];
                      
                      // Find user's active stage
                      const activeStageObj = sortedEtapas.find(stage => {
                        const stageTasks = activeProcesos.filter((p: any) => p.etapa_id === stage.id);
                        if (stageTasks.length === 0) return false;
                        return !stageTasks.every((t: any) => userCompletedTasks.includes(t.id));
                      });
                      const activeStageId = activeStageObj ? activeStageObj.id : (sortedEtapas[sortedEtapas.length - 1]?.id || null);

                      return sortedEtapas.map((stage) => {
                        const stageTasks = activeProcesos.filter((p: any) => p.etapa_id === stage.id);
                        const isStageCompleted = stageTasks.length > 0 && stageTasks.every((t: any) => userCompletedTasks.includes(t.id));
                        const isStageActive = stage.id === activeStageId;

                        let stepClass = "";
                        if (isStageCompleted) stepClass = styles.timelineStepCompleted;
                        else if (isStageActive) stepClass = styles.timelineStepActive;

                        return (
                          <div key={stage.id} className={`${styles.timelineStep} ${stepClass}`}>
                            <div className={styles.timelineStepIcon}>
                              {isStageCompleted ? "✓" : stage.orden_secuencial}
                            </div>
                            <div className={styles.timelineStepContent}>
                              <div className={styles.timelineStepTitle}>{stage.nombre_etapa}</div>
                              
                              {(isStageActive || isStageCompleted) && stageTasks.length > 0 && (
                                <div className={styles.timelineStepTasks}>
                                  {stageTasks.map((t: any) => {
                                    const isDone = userCompletedTasks.includes(t.id);
                                    return (
                                      <div key={t.id} className={styles.timelineTask}>
                                        <div className={styles.taskCheckbox}>
                                          <span>{isDone ? "☑" : "☐"}</span>
                                          <span style={{ textDecoration: isDone ? "line-through" : "none", color: isDone ? "#94a3b8" : "inherit" }}>
                                            {t.nombre_tarea}
                                          </span>
                                        </div>
