        ...
        <section className={styles.profileCard}>
          ...
          {isEditing ? (
            ...
            <div className={styles.userInfo}>
              ...
              {/* Bloque Directo: Gestionar Contactos de Miembros de Familia / Hijos */}
              {familia.filter(f => f.id !== currentUser.persona_id).length > 0 && (
                <div style={{ marginTop: '1rem', padding: '1rem', background: '#f8fafc', borderRadius: '10px', border: '1px solid #e2e8f0' }}>
                  <h3 style={{ fontSize: '0.95rem', color: '#1e293b', margin: '0 0 0.4rem 0', display: 'flex', alignItems: 'center', gap: '0.35rem', fontWeight: 700 }}>
                    📱 Datos de Contacto de mis Hijos / Familiares
                  </h3>
                  <p style={{ fontSize: '0.82rem', color: '#64748b', margin: '0 0 0.85rem 0', lineHeight: 1.3 }}>
                    Asigna o edita aquí el teléfono y correo de tus hijos.
                  </p>
                    ...
                    {familia.filter(f => f.id !== currentUser.persona_id).map((fam) => (
                      <div key={fam.id} style={{ background: 'white', padding: '0.75rem', borderRadius: '8px', border: '1px solid #cbd5e1' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.4rem' }}>
                          <strong style={{ fontSize: '0.88rem', color: '#0f172a' }}>{fam.nombre}</strong>
                          {fam.rol_familiar && <span style={{ fontSize: '0.7rem', backgroundColor: '#e2e8f0', color: '#334155', padding: '1px 6px', borderRadius: '4px', fontWeight: 600 }}>{fam.rol_familiar}</span>}
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '0.5rem' }}>
                          <div>
                            <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#475569', marginBottom: '0.2rem' }}>Teléfono</label>
                        ...
                        <div style={{ marginTop: '0.5rem', textAlign: 'right' }}>
                          <button
                            ...
                            onClick={async (e) => {
                              const btn = e.currentTarget;
                              const telInput = document.getElementById(`fam_main_tel_${fam.id}`) as HTMLInputElement;
                              const emailInput = document.getElementById(`fam_main_email_${fam.id}`) as HTMLInputElement;
                              const newTel = telInput?.value;
                              const newEmail = emailInput?.value;
